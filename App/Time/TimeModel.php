...
class TimeModel
{
    ...
    public function getWeekData (array $week_ts): array {
        ...
        foreach($week_ts as $day_ts => $date) {
            ...
            $params = ['auth_id' => $this->auth_id, 'begin_date' => $date];
            $week_data[$day_ts] = $db->prepareFetchAll($sql, $params);
        }
    ...
    public function getWeekTimes (array $week_ts): array {
        ...
        $week_user_total = 0;
        foreach($week_ts as $day_ts => $date) {
            $task_time_user_on_day = $this->sumTime(['begin_date' => $date, 'auth_id' => $this->auth_id]);
